...
const controller = ({ strapi }: { strapi: Core.Strapi }) => ({
  async generate(ctx: any) {
    const { uid, documentId, locale, pageInstructions, selectedFields, schemaTypes, customSchemas } = ctx.request.body;
    ...
    try {
      const result = await service.generateSeoTags({ uid, documentId, locale, pageInstructions, selectedFields, schemaTypes, customSchemas });
      ctx.body = result;
    } catch (error: any) {
